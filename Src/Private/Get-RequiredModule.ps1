function Get-RequiredModule {
    ...
    Process {
        ...
        if ($ModuleVersion -eq ".")  {
            throw "VMware PowerCLI $Version or higher is required to run the VMware ESXi As Built Report. Run 'Install-Module -Name $Name -MinimumVersion $Version' to install the required modules."
        }
        if ($ModuleVersion -lt $Version) {
            throw "VMware PowerCLI $Version or higher is required to run the VMware ESXi As Built Report. Run 'Update-Module -Name $Name -MinimumVersion $Version' to update the required modules."
        }
    }
    End {}
}